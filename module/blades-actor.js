...
export class BladesActor extends Actor {
  ...
  async rollAttributePopup(attribute_name) {

    // const roll = new Roll("1d20 + @abilities.wis.mod", actor.getRollData());
    let attribute_label = BladesHelpers.getAttributeLabel(attribute_name);
    let is_action = BladesHelpers.isAttributeAction(attribute_name);
    let is_simple = false;
    let actor_id = this.id;
    let is_disabled = true;
    let hbContent = await renderTemplate("systems/blades-in-the-dark/templates/roll-dialog.html", {attribute_name, attribute_label, is_action, is_simple, actor_id, is_disabled});
    ...
    const rollDialog = class extends Dialog{
      constructor(actor, html){
        super(html)
        this.actor = actor
      }

      //figure out how to enable/disable the buttons depending on whether the data above has been selected
      activateListeners(html){
        super.activateListeners(html);
        html.find("button.roll-mod").click(async ev => {
          let modifier = ev.currentTarget.dataset.modifier;
          let position = html.find('[name="pos"]:checked')[0].value;
          let effect = html.find('[name="effect"]:checked')[0].value;
          await this.actor.rollAttribute(attribute_name, modifier, position, effect);
          this.close();
        });
      }
    }
  ...
  async rollAttribute(attribute_name = "", additional_dice_amount = 0, position, effect) {
    ...
    dice_amount += parseInt(additional_dice_amount);
